...
> ~~Best 12h model: in2d_out12h (Val Loss=0.2842)~~ → **아래 아키텍처 비교 참조**

### 아키텍처 비교 결과

#### 12h 예측 — 모델별 성능

| 모델 | Best Epoch | Val Loss | Val MAE | Val RMSE | Gap (best) |
|------|-----------|----------|---------|----------|-----------|
| **GNN+Transformer** | **12** | **0.2746** | **0.3884** | **0.5014** | **+0.005** |
| Transformer (baseline) | 12 | 0.2842 | 0.3932 | 0.5055 | +0.024 |
| GNN+BiLSTM | 10 | 0.2921 | 0.4009 | 0.5173 | +0.010 |
| GNN+TCN | 30 | 0.3082 | 0.4138 | 0.5305 | +0.030 |
| TCN | 28 | 0.3273 | 0.4268 | 0.5437 | +0.062 |
| TimesNet | 7 | 0.4219 | 0.4666 | 0.5939 | +0.157 |

> **Best 12h model: GNN+Transformer** (Val Loss=0.2746, Val MAE=0.3884)

#### 24h 예측 — 모델별 성능

| 모델 | Best Epoch | Val Loss | Val MAE | Val RMSE | Gap (best) |
|------|-----------|----------|---------|----------|-----------|
| **GNN+Transformer** | **14** | **0.5629** | **0.5096** | **0.6580** | **-0.011** |
| Baseline (no reg) | 2 | 0.5845 | 0.5174 | 0.6681 | +0.003 |
| GNN+BiLSTM | 18 | 0.5957 | 0.5275 | 0.6804 | +0.035 |
| C2 (Transformer+reg) | 12 | 0.6030 | 0.5209 | 0.6731 | +0.025 |
| GNN+TCN | 17 | 0.6075 | 0.5542 | 0.7068 | -0.028 |
| TCN deep (4L) | 8 | 0.6231 | 0.5409 | 0.6961 | +0.005 |
| TCN (3L) | 13 | 0.6414 | 0.5471 | 0.7029 | +0.009 |
| TimesNet | 3 | 0.7540 | 0.5860 | 0.7474 | +0.130 |

> **Best 24h model: GNN+Transformer** (Val Loss=0.5629, Val MAE=0.5096)

#### 아키텍처 비교 분석

1. **GNN+Transformer가 전체 최고** — 12h(0.2746)과 24h(0.5629) 모두에서 SOTA. 24h에서 기존 val_loss 0.60 벽을 처음으로 돌파.
2. **GNN의 과적합 억제 효과** — gap(best)이 Transformer 단독(0.024) 대비 GNN+Transformer(0.005)로 대폭 감소. 변수 그룹화가 structural regularizer 역할.
3. **Temporal encoder 순위: Transformer > BiLSTM > TCN** — GNN 내에서 일관된 경향. Transformer의 전역 attention이 GNN graph features와 최적 결합.
4. **TCN 단독은 Transformer 대비 열세** — 12h 15%, 24h 6% 열세. Deep TCN(4L)이 3L보다 나으나 여전히 부족.
5. **TimesNet은 이 과제에 비효과적** — FFT 기반 주기 탐지가 비주기적 폭풍 데이터에 근본적 미스매치. 12h/24h 모두 최하위, 극심한 과적합.
...
| 2025-04-08 | 전체 아키텍처 비교 결과 기록. GNN+Transformer가 12h(0.2746)·24h(0.5629) 모두 SOTA. TimesNet 최하위 확인 |
